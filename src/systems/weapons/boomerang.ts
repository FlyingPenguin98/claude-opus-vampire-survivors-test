...
/** Throws projectiles that fly out, then curve back through the player (hit both legs). */
export class BoomerangBehavior implements WeaponBehavior {
  ...
  private fire(inst: WeaponInstance, ctx: WeaponContext): boolean {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!proj) break;
      proj.fire(player.x, player.y, baseAngle + offset, speed, dmg, inst.pierce, inst.def.textureKey, inst.tint, inst.element, 1800, 'boomerang', player);
    }
    return true;
  }
}
